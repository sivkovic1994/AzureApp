...
| Secrets | **Azure Key Vault** | SQL and Application Insights connection strings are stored as Key Vault secrets; the API's system-assigned managed identity reads them at startup (RBAC-scoped to "Key Vault Secrets User") — no credentials in source control, app settings, or plain environment variables |
| Monitoring | **Application Insights** | Wired up via the Azure Monitor OpenTelemetry SDK — every request, dependency call (SQL), and unhandled exception is captured and queryable |
...
- Docker
- Azure Container Apps, Azure SQL Database, Azure Static Web Apps, Azure Key Vault, Application Insights
